#!/usr/bin/env python3
"""Validate workspace testcase index for the test-case-generator skill."""
...
def main() -> int:
    if len(sys.argv) != 2:
        print('Usage: validate_testcase_index.py <path/to/testcases/index.json>')
        return 1
    ...
    for idx, entry in enumerate(entries, start=1):
        ...
        for key in ['id', 'title', 'module', 'topic', 'format', 'rel_path', 'status', 'created_at', 'updated_at']:
            try:
    ...
    print(f'OK: validated testcase store with {len(entries)} entries')
    return 0
