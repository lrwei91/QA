...
当用户选择「导出为 Excel 文件」时，必须调用本 skill，不得手动创建 Excel 文件。
...
python3 test-case-generator/scripts/upsert_testcase_index.py \
    testcases/generated/module.xlsx
```
...
```bash
# 索引校验
python3 test-case-generator/scripts/validate_testcase_index.py \
...
# 索引差异比较
python3 test-case-generator/scripts/diff_testcase_indexes.py \
    old-index.json new-index.json

# 覆盖率报告
python3 test-case-generator/scripts/export_testcase_report.py \
    testcases/testcase-index.json --output report.md
```
...
```
testcases/generated/<模块>/<用例名称>.xlsx
例：testcases/generated/认证中心/用户登录功能优化.xlsx
```
...
例：testcases/i18n/HeroSkill/HeroSkill-SkillUpgrade.json
```
...
文件路径：testcases/generated/<模块>/<用例名称>.xlsx
用例数量：XX 条
- 账服：XX 条
- 客户端：XX 条

索引已更新：testcases/testcase-index.json
```
...
原文件：testcases/generated/<模块>/<用例名称>.xlsx
新增用例：XX 条
- 新增行已标黄标识
- 索引已更新 updated_at
```
