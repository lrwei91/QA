...
11. 若需求或对话中包含多语言文案，检查是否覆盖固定语言集合；仅在语言值完整时生成多语言 JSON，不完整时输出缺失语言清单。
12. 若用户要求导出或落盘，按模板保真、默认留空、Excel 展示规则和工作区索引规则写出结果。
13. 只有在用户明确要求“更新 skill / 更新索引 / 修改模块文件”时，才进入维护模式并修改资源文件。
