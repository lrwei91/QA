...
必须使用 `Office-Skills/skills/minimax-xlsx` 的模板和脚本导出 Excel，不得使用 openpyxl 直接创建。

```bash
# 使用 minimax-xlsx 脚本
python3 Office-Skills/skills/minimax-xlsx/scripts/xlsx_fill_testcase_template.py \
    rows.json output.xlsx --template Office-Skills/skills/minimax-xlsx/templates/testcase_template.xlsx
```
...
- 多语言 JSON 合法性优先使用 `scripts/validate_i18n_json.py` 校验。
