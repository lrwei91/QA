...
测试用例导出时，**自动同时生成 MD 和 Excel 两种格式**，无需用户选择：

1. **MD 文档** → `knowledge/wiki/testcases/<模块>/<用例名称>.md`
2. **Excel 文件** → `outputs/generated/<模块>/<用例名称>.xlsx`

### MD 导出

使用 `templates/testcase_template.md` 模板和 `generate_testcase_md.py` 脚本：

```bash
python3 engine/scripts/generate_testcase_md.py \
    rows.json output.md --template templates/testcase_template.md \
    --module <模块名> --title <用例标题> \
    --excel outputs/generated/<模块>/<用例名称>.xlsx \
    --snapshot outputs/snapshots/<模块>/<用例名称>/
```
...
| `{generated_at}` | 生成时间 |
| `{total_count}` | 用例总数 |
| `{server_cases}` | 账服用例表格行 |
| `{client_cases}` | 客户端用例表格行 |
| `{excel_path}` | Excel 文件相对路径 |
| `{snapshot_path}` | 快照目录相对路径 |
...
| 脚本 | 用途 |
|------|------|
| `generate_testcase_md.py` | 生成 MD 测试用例文档 |
