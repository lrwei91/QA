...
# 纳入单个测试用例
python3 upsert_testcase_index.py outputs/generated/运营活动/任务系统前后端优化.xlsx

# 纳入单个多语言 JSON
python3 upsert_testcase_index.py outputs/i18n/个人中心/个人中心 - 免费旋转记录.json
```
...
```bash
python3 validate_testcase_index.py outputs/testcase-index.json
```
...
```bash
python3 validate_i18n_index.py outputs/i18n-index.json
```
...
```bash
python3 validate_i18n_json.py outputs/i18n/个人中心/个人中心 - 免费旋转记录.json
```
...
python3 diff_testcase_indexes.py \
    ...
    outputs/testcase-index-backup.json
```
...
python3 generate_testcase_from_template.py \
    ...
    --output outputs/generated/模块/用例名称.xlsx \
    --data rows.json
```
...
```bash
# 解析单个 HTML 文件
python3 parse_axure_html.py outputs/axure_export/login_page.html

# 解析目录下的所有 HTML 文件
python3 parse_axure_html.py outputs/axure_export/

# 递归解析子目录
python3 parse_axure_html.py outputs/axure_export/ --recursive
...
```bash
# 导出覆盖率报告
python3 export_testcase_report.py --output outputs/coverage_report.md
```
