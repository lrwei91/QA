...
**子能力编排入口（仅供内部参考，不可直接调用）：**
- `testcase-generate` - 生成测试用例（由 /qa 内部调用）
- `testcase-augment` - 补充已有用例（由 /qa 内部调用）
- `testcase-format` - Excel 导出与索引更新（由 /qa 内部调用）
- `figma-reader` - Figma 设计稿读取（由 /qa 内部调用）
- `axure-parser` - Axure HTML 解析（由 /qa 内部调用）
- `knowledge` - 知识库管理（消化/查询/结晶）
- `zentao` - 禅道集成（bug/任务/需求/测试用例）

**重要：** 子能力仅供 `/qa` 内部调度使用，用户不能直接调用 `/testcase-format` 等命令。

---

## 子能力说明

**重要：本项目的"子能力"是能力模块，不是独立的命令行子命令。**

- **Skill = 能力说明书**：每个子能力对应一个 `SKILL.md` 文档，定义了该能力的职责、输入输出、行为规则
- **由 `/qa` 统一调度**：用户只需与 `/qa` 交互，`/qa` 根据用户选择的场景（生成/补充/导出等）调用相应的 Skill
- **不可独立执行**：不能直接运行 `/qa testcase-generate` 这类命令，必须通过 `/qa` 菜单式入口选择操作

**架构层次：**
```
用户交互层 (/qa 命令)
       ↓
编排调度层 (根据选择调用对应 SKILL.md)
       ↓
能力执行层 (Claude 参照 SKILL.md 执行任务)
       ↓
脚本工具层 (Python 脚本负责文件操作、数据处理)
```
