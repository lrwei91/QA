...
# 但 Python 检查失败
python3 -c "from pathlib import Path; print(Path('testcases/generated/无 CPF 用户限制功能.xlsx').exists())"
```
...
```python
import unicodedata
path = unicodedata.normalize('NFC', 'testcases/generated/无 CPF 用户限制功能.xlsx')
# 然后使用规范化后的路径
```
...
**错误信息：**
```
ERROR: file not found: /path/to/testcases/generated/无 CPF 用户限制功能.xlsx
```
但文件实际存在于该路径。
...
```bash
# 使用双引号包裹完整路径
python3 test-case-generator/scripts/upsert_testcase_index.py "testcases/generated/无 CPF 用户限制功能.xlsx" --topic "xxx" --module "xxx"
```
