...
resolve_skill_root() {
  ...
  # 对于 knowledge 目录，直接返回 PROJECT_ROOT（即 knowledge/）
  # 这样 deps/baoyu-url-to-markdown 路径才能正确解析
  printf '%s\n' "$PROJECT_ROOT"
}
...
dependency_installed() {
  ...
  case "$dependency_type" in
    bundled)
      ...
      # bundled 依赖位于 deps/{dependency_name}/ 目录下
      if [ -d "$skill_root/deps/$dependency_name" ]; then
        return 0
      fi
      return 1
      ;;
...
default_install_hint() {
  ...
  case "$source_id" in
    web_article)
      printf '%s\n' "重新运行 knowledge/install.sh --platform claude，确认 ${adapter_name} 已准备到技能目录"
      ;;
    *)
      printf '%s\n' "-"
      ;;
